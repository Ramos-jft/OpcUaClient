...
internal sealed class OpcUaReadService
{
    ...
    public IReadOnlyList<OpcNodeValue> ReadNodes(
        Session session,
        IReadOnlyList<(NodeId NodeId, string DisplayName)> nodes)
    {
        ...
        ReadValueIdCollection readValueIds = CreateReadValueIds(nodes);
        ...
        return CreateNodeValues(nodes, results);
    }

    private static ReadValueIdCollection CreateReadValueIds(
        IReadOnlyList<(NodeId NodeId, string DisplayName)> nodes)
    {
    ...
    private static IReadOnlyList<OpcNodeValue> CreateNodeValues(
        IReadOnlyList<(NodeId NodeId, string DisplayName)> nodes,
